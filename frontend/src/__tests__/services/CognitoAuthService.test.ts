...
describe('CognitoAuthService', () => {
  ...
  describe('getTokens', () => {
    ...
    test('returns tokens when stored in cookies', async () => {
      ...
      const payload = btoa(
        JSON.stringify({ sub: 'user-123', email: 'test@gov.bc.ca', exp: futureExp }),
      );
    ...
    test('automatically refreshes tokens when expiring soon', async () => {
      ...
      const header = btoa(JSON.stringify({ alg: 'RS256', typ: 'JWT' }));
      const payload = btoa(
        JSON.stringify({ sub: 'user-123', email: 'test@gov.bc.ca', exp: nearExp }),
      );
      const signature = 'signature';
      ...
      const newPayload = btoa(
        JSON.stringify({ sub: 'user-123', email: 'test@gov.bc.ca', exp: newFutureExp }),
      );
