...
const TestComponent: React.FC<ComponentProps> = ({ user, testName, questionFileName }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const adminReportStatus = await sendAdminReport(
      displayName,
      userEmail,
      percentage,
      testName,
    ...
    setEmailStatus(
      userReportStatus === 'success' && adminReportStatus === 'success' ? 'success' : 'error',
    );
    setIsLoading(false);
  };
    ...
    if (isSubmitted && areAnswersComplete(userAnswers)) {
      ...
      return (
        <p className={isPassing(questions, userAnswers) ? 'text-success' : 'text-danger'}>
          {isPassing(questions, userAnswers)
            ? 'Congratulations! You have passed'
            : 'Sorry! You have failed'}{' '}
          with {scorePercentage}%.
        </p>
      );
    }
    return null;
          ...
          {questions.length > 0 ? (
            <div className="container mb-5">
              ...
              <h4 className="pt-2">
                ...
                , welcome to the {testName} for the RESULTS application access.
              </h4>
                  ...
                  <div key={index} className="mt-5 unselectable">
                    ...
                    <ul className="list-group">
                      {question.choices.map((choice, choiceIndex) => (
                        <li
                          key={choiceIndex}
                          className={`list-group-item ${isSubmitted && userAnswers[index] === choiceIndex ? (choice.isCorrect ? 'list-group-item-success' : 'list-group-item-danger') : ''}`}
                        >
                          <label>
                            <input
                              type="radio"
